...
public class NameMainActivity extends AppCompatActivity {
    ...
    ArrayList<NameGetSet> list = null;
    NameGetSet nameDrug = null;
    String imag;
    String key = "jfITbHJfFcUac1Slfs9wJebHSiGh29B5IXR2P3M0tqLKEW86i%2B92MKTAZsYyEatYE3bMFJpG1C5LnoXceDThvw%3D%3D"; //약국 공공데이터 서비스키
    ...
    public class MyAsyncTask extends AsyncTask<String, Void, String> {//공공데이터 파싱하는 코드. 배열에 넣어 불러옴
        ...
        protected String doInBackground(String... strings) {//전달된 URL을 사용하는 작업을 한다
            ...
            try {
                ...
                //공공데이터 파싱을 위한 주소
                requestDrugUrl = "https://apis.data.go.kr/1471000/MdcinGrnIdntfcInfoService01/getMdcinGrnIdntfcInfoList01?serviceKey="//요청 URL
                        + key  + "&numOfRows=3&item_name=" + drugSearch; //약 이름으로 검색 하기.

                //실질적으로 파싱해서 inputstream해주는 코드
                URL url = new URL(requestDrugUrl); //공공데이터 파싱 주소를 url에 넣음
