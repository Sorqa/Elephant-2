...
public class ShapeSearch extends AppCompatActivity {
    private RecyclerView recyclerView;
    ArrayList<ShapeGetSet> list = null;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.shape_list);
        ...
        mAdapter = new ShapeAdapter(getApplicationContext(), list);
    ...
    public void searchJson(){
        try{
            InputStream is = getAssets().open("shapelist.json"); //assets파일에 저장된 json 파일 열기
            ...
            JSONArray jsonArray = jsonObject.getJSONArray("shapelist"); //json파일에서 의약품리스트의 배열명, jsonArray로 저장
            ...
            for(int i=0; i<jsonArray.length(); i++){
                ...
                if(choosecolor != null && chooseshape == null && choosetype == null){
                    if ((jsonObject.getString("색상앞").contains(choosecolor))) {
                        ShapeGetSet formDrug = new ShapeGetSet();
